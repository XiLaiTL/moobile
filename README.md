...
```moonbit
// moon.mod:  import { "XiLaiTL/moobile@0.1.0" }
// moon.pkg:  import { "XiLaiTL/moobile" @moobile, "XiLaiTL/moobile/style",
//                     "XiLaiTL/moobile/html", "XiLaiTL/moobile/cmd" }
...
> 缺哪个组件，对应标签就会变成 `undefined` —— 标签表的值域就是这 5 个名字。
...
| 样式 | 类型化 `StyleValue`；`class=` 与 `style="…"` 在 RN 上**不生效**（前者 RN 无类名，后者会被类型化样式覆盖） |
...
| `_tools/` | 环境脚本 + `vendor_sync.sh` / `patches/`（见下） |
| `_verify.js` `_r1.js` | 验证与测量脚本 |
| `_r1/` | R1 的测量产出与原始数据 |

**第三方（rabbita fork）不进仓**：`html/` `cmd/` `dom/` `js/` `internal/` `sub/` `svg/` … 共 16 个目录
是**生成物**，由 `_tools/vendor.lock` 的版本 + `_tools/patches/` 的 15 个 patch 重建。
原因：MoonBit 的 `internal` 可见性按包路径前缀判，fork 必须铺在模块根；
而"别人的代码不该进我们的仓"。机制见 [`FORK.md`](FORK.md)。
...
环境细节、真机（模拟器）流程、排错表、**禁区** → [`DEV.md`](DEV.md)。
